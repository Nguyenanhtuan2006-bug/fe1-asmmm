...
export class Detail implements OnInit {
  ...
  addToCart() {
    ...
    if (!userString) {
      alert('Bạn đăng nhập để thêm vào giỏ hàng!');
    ...
    let index = cart.findIndex(
      (item: any) => item.id === this.product.id && item.userId === currentUserId,
    );

    if (index !== -1) {
      cart[index].quantity += this.quantity;
    } else {
      cart.push({ ...this.product, quantity: this.quantity, userId: currentUserId });
    }
